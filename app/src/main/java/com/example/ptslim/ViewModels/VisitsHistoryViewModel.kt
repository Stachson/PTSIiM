...
class VisitsHistoryViewModel {
    ...
    fun getVisitsHistory(dataLoadListener: DataLoadListener) {
        Thread {
            DatabaseConnection.connectToDatabase { connection ->
                if (connection != null) {
                    try {
                        val statement = connection.createStatement()
                        val query = VisitsMethods.get_my_visits(GlobalData.sharedUserID)
                        println("AAAAAAAAAAAAAAAAAA ${GlobalData.sharedUserID}")
